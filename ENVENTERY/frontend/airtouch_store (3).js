...
const AirTouchStore = (() => {
    ...
    function recordDispatch(city, employee, date, lines, remarks) {
        ...
        // Only check the item exists — stock is allowed to go negative
        // (0, minus, or plus) when dispatching more than what's available.
    ...
    function updateDispatchBatch(batchRef, city, employee, date, lines, remarks) {
        ...
        // Only check the item exists — stock is allowed to go negative here too.
        for (const line of lines) {
            const item = _items.find(i => i.id == line.itemId);
